...
void setup()
{
    ...
    targetHeading = 120.0*(M_PI/180.0);
    targetSpeed = 20;
    targetAltitude = -300;
...
void loop()
{
    ...
    if(time >= nextWrite) {
        ...
        if (counter <= 10){
            ...
            counter = 0;
            targetHeading = targetHeading + 90.0*(M_PI/180.0);
        }
...
#define SERVO_MAX 2000 // Maximum duty cycle
        ...
        if(time >= nextPrint) {
            ...
            float al_error = targetAltitude-pnPePd.z;
            float cl_error = altitudePIDOut-pnPePdDot.z;
            float pi_error = climbRatePIDOut-phiThetaPsi.y;
