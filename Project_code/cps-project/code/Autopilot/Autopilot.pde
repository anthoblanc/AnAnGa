...
void loop()
{
    ...
    if(time >= nextWrite) {
		...
		// Updating the Aerobatic Trajectory Controller
			...
			struct vector aCMD_refin, gCMD_refin, aCMD_refin, gCMD_refin, eulerDesired = {0,0,0};
			int8_t aerobatOn = 0;
			...
			stSig = trCTRL.update(deltaL,aCMD_refbody,gCMD_refbody,phiRef,aerobatO_refin,eulerDesired);
