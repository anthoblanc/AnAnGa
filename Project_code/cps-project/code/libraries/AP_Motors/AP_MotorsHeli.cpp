...
void AP_MotorsHeli::move_swash(int16_t roll_out, int16_t pitch_out, int16_t coll_in, int16_t yaw_out)
{
    ...
    if(!_servo_manual) {
        ...
        // Manual control
        hal.rcout->write(0, hal.rcin->read(2));
        hal.rcout->write(1, hal.rcin->read(0));
        hal.rcout->write(2, hal.rcin->read(1));
        hal.rcout->write(3, hal.rcin->read(3));
    }
...
// Perform a switch to/from manual commands
void AP_MotorsHeli::set_manual_commands(bool manual)
{
    _servo_manual = manual == 1;
}
